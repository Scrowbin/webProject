...
$user_avatar = '/assets/static/avatar_default.png'; // Path relative to root
...
if ($is_logged_in && isset($_SESSION['userID'])) {
    ...
    if ($user_data && !empty($user_data['Avatar']) && $user_data['Avatar'] !== 'avatar_default.png') {
        $user_avatar = '/uploads/avatars/' . htmlspecialchars($user_data['Avatar']);
    }
}
?>
...
<nav class="navbar navbar-minimal">
  <div class="container">
    ...
    <a class="navbar-brand text-white fw-bold d-flex align-items-center" href="/">
       <img src="/assets/static/logo.png" alt="<?= htmlspecialchars(site_name()) ?> logo" style="height: 30px; margin-right: 8px;">
       <span>
         <?= htmlspecialchars(site_name()) ?>
         <span class="navbar-brand-tagline"><?= htmlspecialchars(site_title()) ?></span>
       </span>
    </a>
      ...
      <div class="modal-body user-modal-body p-3">
        ...
        <?php if ($is_logged_in): ?>
          ...
          <div class="d-flex justify-content-between mb-2 user-modal-actions px-2">
            <?php // Link to profile controller action ?>
            <a href="/profile" class="text-decoration-none text-white d-flex align-items-center"><i class="bi bi-gear me-2"></i> Settings</a>
          </div>
